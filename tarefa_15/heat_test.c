...
int main(int argc, char *argv[]) {
    ...
    if (rank == 0) {
        printf("Versao   : MPI_Isend / MPI_Irecv + MPI_Test (overlap)\n");
    ...
    for (int step = 0; step < STEPS; step++) {
        ...
        if (rank < size - 1) MPI_Isend(&u[local_n],     1, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD, &reqs[nreqs++]);
        if (rank > 0)        MPI_Irecv(&u[0],            1, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD, &reqs[nreqs++]);
        if (rank > 0)        MPI_Isend(&u[1],            1, MPI_DOUBLE, rank - 1, 1, MPI_COMM_WORLD, &reqs[nreqs++]);
        if (rank < size - 1) MPI_Irecv(&u[local_n + 1], 1, MPI_DOUBLE, rank + 1, 1, MPI_COMM_WORLD, &reqs[nreqs++]);
        ...
        if (local_n > 1)
            u_new[local_n] = u[local_n] + R * (u[local_n - 1] - 2.0 * u[local_n] + u[local_n + 1]);
